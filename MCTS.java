...
public class MCTS {
    ...
    private double time, startTime = 0;
        ...
        for (int i = 0; i < tour.size(); i++) {
            ...
            for (int j = 0; j < stops.size(); j++) {
                ...
                for (Vertex<Customer, Double> k : allStops) {
                    ...
                    if (!k.isVisited()) {
                        z += Math.exp(globalPolicy[stop.getId()][k.vertexInfo.getId()]);
                    }
                }
                for (Vertex<Customer, Double> k : allStops) {
                    if (k.equals(stop)) {
                    ...
                    if (!k.isVisited()) {
                        policy[level][stop.getId()][k.vertexInfo.getId()] -= ALPHA * (Math.exp(globalPolicy[stop.getId()][k.vertexInfo.getId()]) / z);
                    }
                }
    ...
    public ArrayList<Vehicle> rollout() {
        ...
        while (true) {
            ...
            for (int i = 0; i < allStops.size(); i++) {
                if (allStops.get(i).vertexInfo.equals(currentStop)) {
                    continue;
                }
                if (!allStops.get(i).isVisited() && !checked.get(i)) {
                    possible_successors.add(allStops.get(i).vertexInfo);
                }
            }
            if (possible_successors.isEmpty()) {
                if (!currentStop.equals(map.head.vertexInfo)) {
                    newTour.get(newTour.size() - 1).addRoute(map.head.vertexInfo);
                }
