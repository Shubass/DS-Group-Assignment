...
public class customAlgorithm<T extends Comparable<T>> {
    ...
    public customAlgorithm(Graph deliveryGraph, int capacity) {
    ...
    public ArrayList<Vertex> getRoute() {
        ...
        visited.add(temp);
        if (!visited.isEmpty()) {
            while (!open.isEmpty() && maximumCapacity<capacity ) {
                ...
                for (int i = 0; i < open.size(); i++) {
                    if (open.get(i).equals(nextVertex.vertexInfo)) {
                        visited.add(deliveryGraph.getTheVertex(open.get(i)));
                        open.remove(i);
                    }
                }
                temp = nextVertex;
